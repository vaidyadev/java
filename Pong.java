...
class Game extends JPanel implements ActionListener
{
	...
	public void doPaint(Graphics g)
	{
		...
		{ //if the game is in progress, paint the board
        	...
        	g.drawString("Use RIGHT and LEFT", 750,50);
			
			g.setColor(Color.white);
			g.setFont(new Font("MV Boli", Font.BOLD, 30));
        	g.drawString("Use UP and DOWN", 50,50);
	...
	public void gameOver(Graphics g)
	{
	 ...
	 public class KeyMover extends KeyAdapter 
	 {
	        ...
	        public void keyPressed(KeyEvent e) {
	            ...
	            if ((key == KeyEvent.VK_RIGHT) ) { // if the key pressed is Q, move the left paddle up
	               leftup=true;
	               leftdown=false;
	            }

	            if ((key == KeyEvent.VK_LEFT) ) { // if the key pressed is A, move the left paddle down
	               leftup=false;
	               leftdown=true;
	            }
